...
class NvidiaGPUSpecsCrawler(BaseCrawler):
    ...
    COLUMN_NAME_DICT = {
        ...
        'Clock speeds - Base core (GHz)': 'base clock (GHz)',
        'Clock speeds - Boost core (GHz)': 'boost clock (GHz)',
    ...
    def _parse_section(self, soup, header, sel_model_names, append_value_map):
        ...
        for col in [column for column in table.columns if 'Processing power' in column]:
            # Split the string at the space and keep only the first part
        ...
        # convert units
        # mhz -> ghz
        columns_to_convert = ['Clock speeds - Base core (MHz)', 'Clock speeds - Boost core (MHz)']
        for column in columns_to_convert:
            if column in table.columns:
                # Rename the column and convert its values
                new_column_name = column.replace('MHz', 'GHz')
                table[new_column_name] = table.pop(column).astype(float) / 1000
        # gflop -> tflop
        ...
        for column in gflop_columns:
            if column in table.columns:
                ...
                table[new_column_name] = table.pop(column) / 1000
        
        # Rename columns and drop others
        table.rename(columns=self.COLUMN_NAME_DICT, inplace=True)
        table = table.filter(items=self.COLUMN_NAME_DICT.values())
        
        # Append new columns
        table = table.assign(**append_value_map)
        
        # Add a new column "fab (nm)"
        print(table['fab model'])
